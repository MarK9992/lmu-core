...
public class LmuParser extends Analyzer {
					...
					try {
						...
						if (statement.equalsIgnoreCase("show") || statement.equalsIgnoreCase("hide")) {
							...
							if (line.size() < 2) {
								...
								try {
									...
									Set<Entity> srcEntities = Entities
											.findEntityWhoseNameSpaceMatches(model.getEntities(), regexp);
									...
									if (srcEntities.isEmpty()) {
										...
										Set<Entity> entitiesToRemove = (Set<Entity>) Collections
												.difference(model.getEntities(), entitiesToKeep);
							...
							if (line.size() != 1) {
								...
								Collection<Entity> isolatedEntities = Entities.findIsolatedEntities(model.getEntities(),
										model);
							...
							if (currentEntity == null) {
								...
								throw new ParseError(lineNumber,
	...
	public void _has(List<String> line) throws ParseError, ModelException {
		...
		if (currentEntity == null) {
			...
			if (relationType.equals("aggregation")) {
				...
				throw new ParseError(lineNumber, "the <b>'" + relationType
	...
	public void _features(List<String> line) throws ParseError, ModelException {
		if (currentEntity == null) {
			...
			throw new ParseError(lineNumber, "the <b>'features'</b> statement should consist of 7 elements",
			...
			if (visibilityValue.equalsIgnoreCase("public"))
				...
				throw new ParseError(lineNumber, "<b>'" + visibilityValue
			...
			if (entityElement instanceof Attribute) {
				...
				if (line.size() > 7) {
					if (!line.get(7).equalsIgnoreCase("expecting"))
						syntax("the <b>'expecting'</b> keyword was expected but <b>'" + line.get(7)
								+ "'</b> was found");
